...
public class RegisterDao {
	 ...
	 * @return 响应json数据
	 */
	public static JSONObject doPost(JSONObject json) {
		try {
	...
	private static void createAccount(JSONObject json, String account, String password, String edupassword) {
		try {
			if(json.getString("flag").equals("1")) {
				int id = StaticData.getNextId();
				//user表
				PreparedStatement ps = C3P0Util.getConnection().prepareStatement("INSERT INTO user(id,account, password, edupassword) values(?,?,?,?)");
				ps.setInt(1, id);
				...
				//school表
				ps = C3P0Util.getConnection().prepareStatement("INSERT INFO school(id,account) values(?,?)");
				ps.setInt(1, id);
				ps.setString(2, account);
				if(!ps.execute()) json.put("result",false);
				//own表
				...
				ps = C3P0Util.getConnection().prepareStatement("INSERT INFO own(id,head,good,picture) values(?,?,?,?)");
				ps.setInt(1, id);
				ps.setString(2,head);
				ps.setInt(3, 0);
