...
public class Chat {
	...
	public void onOpen(Session session, @PathParam("account") String account) throws IOException, SQLException {
		...
		PreparedStatement ps = C3P0.getConnection().prepareStatement("SELECT * FROM chat WHERE to=? AND state=?");
	...
	public void onMessage(String message) throws IOException, SQLException {
		//发送信息   from,to,msg
		PreparedStatement ps = C3P0.getConnection().prepareStatement("INSERT INTO chat(chatid,from,to,msg,state,date) VALUES(?,?,?,?,?,?)");
